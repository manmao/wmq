...
int client_init(struct sock_client **client,char *server_ip,int port){
...
*
****************************************************************/
int  client_send_data(struct sock_client *client,socket_pkg_t *sendpkt){
		//发送数
		int ret=send(client->conn->fd, sendpkt,sizeof(socket_pkg_t), 0);
		if(ret == -1){
			printf("发送信息失败\n");
			return -1;
		}
...
*
****************************************************************/
int client_recv_data(struct sock_client *client,socket_pkg_t *revpkt){
	 //接收数据
	 int ret=recv(client->conn->fd,revpkt,sizeof(struct socket_pkg), 0);
	 if(ret == -1){
			printf("接收信息失败\n");
			return -1;
	}
