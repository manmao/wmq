...
//初始化客户端
extern int client_init(struct sock_client **client,char *server_ip,int port);				//客户端初始化

//连接到服务器
extern int   start_connection(struct sock_client *client);       				//连接到服务器

extern int  client_send_data(struct sock_client *client,socket_pkg_t *sendpkt);//发送数据到服务器

extern int client_recv_data(struct sock_client *client,socket_pkg_t *revpkt);//接收返回数据
